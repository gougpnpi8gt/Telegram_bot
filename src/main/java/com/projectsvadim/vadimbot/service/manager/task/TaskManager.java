package com.projectsvadim.vadimbot.service.manager.task;
...
public class TaskManager extends AbstractManager {
    ...
    private BotApiMethod<?> sendTask(CallbackQuery callbackQuery, String id, Bot bot) throws TelegramApiException {
        ...
        bot.execute(methodFactory.getCopyMessage(
                task.getStudent().getChatId(),//сообщение отправляется ученику в чат
                chatId,//откуда отправляется
                task.getMessageId(),// какое сообщение
                keyboardFactory.getInlineKeyboard(
